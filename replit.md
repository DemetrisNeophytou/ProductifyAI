...
- **Dashboard Redesign**: Completely redesigned Dashboard with cleaner, Canva/Notion-style layout featuring: Welcome section with personalized greeting, Quick Progress Tracker (compact), Week at a Glance insights (Projects, Views, Exports), and 3 Quick Actions buttons (Create New Product, Generate Content, AI Coach). Additional tools remain accessible via sidebar. Mobile-responsive design with balanced spacing.
- **AI Coach Transformation**: Upgraded to true "Digital Product Coach" with RAG-powered knowledge base. New system includes: human-centered dialogue (2-4 short questions, 3 crisp options max), conversation guardrails (checks user context before answering), RAG knowledge retrieval from 13 expert guides (ebooks, planners, pricing, launch, design, marketplaces), structured outputs (Summary + Next Step + Template + Tool offer), temperature set to 0.4 for consistency. Supports English and Greek responses.
- **RAG Knowledge Base**: Created comprehensive markdown knowledge base in `data/rag/` with 13 expert guides: Recipes (ebooks, planners, workbooks, lead magnets, landing pages, pricing playbook, launch checklist, promo emails, social posts) and Design (font pairings, color accessibility, Gumroad/Etsy listings). Lightweight retrieval service with keyword-based scoring returns top-3 relevant context chunks.
